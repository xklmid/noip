...
if pgrep -x "noip2" >/dev/null 2>&1; then
    read -rp "检测到 noip2 已运行，是否重新安装？(y/n): " answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo "退出安装。"
        exit 0
    fi
...
if command -v apt >/dev/null 2>&1; then
    sudo apt update -y
    sudo apt install -y build-essential wget cron
elif command -v yum >/dev/null 2>&1; then
    sudo yum update -y
    sudo yum groupinstall "Development Tools" -y
    sudo yum install -y wget cronie
else
    echo "错误：不支持的Linux发行版，请手动安装依赖。"
    exit 1
fi
...
if [ ! -f "noip-3.3.0.tar.gz" ]; then
    sudo wget -O noip-3.3.0.tar.gz https://www.noip.com/client/linux/noip-3.3.0.tar.gz
fi
...
# 配置强制刷新脚本
echo "[5/6] 配置定时刷新脚本..."
sudo tee /usr/local/bin/noip-force-update.sh >/dev/null <<EOF
#!/bin/bash
if ! pgrep -x "noip2" >/dev/null 2>&1; then
    echo "\$(date '+%Y-%m-%d %H:%M:%S') No-IP未运行，启动..." >> /var/log/noip-refresh.log
    /usr/local/bin/noip2
else
    echo "\$(date '+%Y-%m-%d %H:%M:%S') No-IP运行中，发送刷新请求..." >> /var/log/noip-refresh.log
    killall -HUP noip2
fi
EOF
...
# 添加cron任务，每5分钟执行一次
( crontab -l 2>/dev/null | grep -v 'noip-force-update.sh'; echo "*/5 * * * * /usr/local/bin/noip-force-update.sh" ) | crontab -

# 配置开机自检脚本
echo "[6/6] 配置开机自检服务..."
sudo tee /usr/local/bin/noip-check-start.sh >/dev/null <<EOF
#!/bin/bash
if ! pgrep -x "noip2" >/dev/null 2>&1; then
    echo "\$(date '+%Y-%m-%d %H:%M:%S') No-IP未运行，尝试启动..." >> /var/log/noip-check.log
    /usr/local/bin/noip2
else
    echo "\$(date '+%Y-%m-%d %H:%M:%S') No-IP运行正常。" >> /var/log/noip-check.log
fi
EOF

sudo chmod +x /usr/local/bin/noip-check-start.sh

sudo tee /etc/systemd/system/noip-check.service >/dev/null <<EOF
[Unit]
